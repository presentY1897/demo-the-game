...
- 각 워크스페이스에 `lint` 스크립트 연결. 규칙은 recommended 수준 + 프로젝트 규칙
  ...
  이벤트 JSON.parse 금지.
- 구현 메모: 커스텀 2종은 로컬 플러그인 `@thegame`
  (`packages/config/eslint/plugin.js`)으로 넣었다. `no-realtime-event-parse`는
  error, `no-hardcoded-ui-string`은 오탐 여지가 있어 warn.
  `rn` 프리셋에는 jsx-a11y를 넣지 않는다 — DOM role/aria 기준이라 RN 컴포넌트에는
  대부분 오탐이고, RN 접근성(accessibilityLabel 등)은 S06에서 따로 다룬다.
  `packages/config` 자신도 lint 대상에 포함(프리셋·규칙이 코드이기 때문).
...
| `mock-server` 봇 | staff 실존 방에서 봇 침묵 (S01 회귀 테스트와 동일 — **작성 주체는 S01**) |
| `tokens` 대비 | S06의 AA 대비 검사 (**작성 주체는 S06**) |
...
2. PR에서 lint/typecheck/test/build가 자동 실행되고 실패 시 머지가 막힌다.
   - 워크플로까지가 이 명세의 범위다. "머지가 막힌다"는 원격 저장소 쪽 브랜치 보호
     규칙(required status check: `lint · typecheck · test · build`) 설정이 전제이고,
     이 저장소는 아직 remote가 없어 미설정 상태다 — S07(배포)에서 remote가 생길 때 함께 건다.
...
- 메타 검증: 의도적으로 실패하는 커밋 1건으로 lint/typecheck/test/build 각 잡이 실제로
  머지를 막는지 확인 후 되돌린다. remote가 없는 동안은 로컬 결함 주입으로 대신한다 —
  네 태스크 각각에 위반을 하나씩 심고 `turbo run lint typecheck test build`가
  non-zero로 끝나는지 확인한 뒤 되돌린다 (커밋하지 않는다).
