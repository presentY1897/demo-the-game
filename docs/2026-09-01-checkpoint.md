...
| product `/ko` 전송량 / 요청 | 546KB / 30 | **155KB / 15** | perf/002 |
...
**한글 브랜드 서체 서브셋 자체 호스팅** — 실제로 만들어 붙였다가 원복했다.
크기는 문제가 아니었다(전체 2,009KB · 한글 전체 서브셋 1,695KB · 제거 전 실제 수신량
390KB/15요청 · 렌더되는 355자만 서브셋 **81KB/1요청**). 문제는 **적용 범위**였다 —
Pretendard는 `packages/tokens`의 공유 디자인 토큰이라 product·Storybook·live-demo가
함께 소비하는데, "쓰는 글자만" 서브셋은 한 화면의 카피에 고정되므로 product에만 붙는다.
같은 브랜드 서체가 화면마다 달라지는 건 토큰 단일 원천 원칙과 어긋난다.
근거와 재측정치는 [perf/002 후속 검토](./perf/002-product-lighthouse.md).
...
1. **README 데모 GIF** (S11) — 미작성. 배포는 완료다: 4개 링크 동작, 배포 환경 스모크
   10/10, CORS 잠금 적용·검증(허용 오리진만 통과, 낯선 오리진 차단, 데모는 정상 동작),
   콜드스타트 실측 — 잠든 뒤 첫 요청 **10.2초**, 이어진 요청 0.20초.
